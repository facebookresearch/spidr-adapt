...
if __name__ == "__main__":
    ...
    parser.add_argument("configs", type=Path, nargs="+", help="TOML config file(s) for the finetuning run(s).")
    ...
    def launching_finetuning(cfg: Config) -> None:
        os.environ["CUDA_VISIBLE_DEVICES"] = os.environ["SLURM_LOCALID"]
        train(cfg)

    if args.dump is None:
        raise ValueError("Dump directory must be specified with --dump or by setting $METASR_SUBMITIT_FOLDER")
    init_logger()

    jobs = [(launching_finetuning, (read_config(cfg),)) for cfg in args.configs]
